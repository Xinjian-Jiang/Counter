...
template <class W>
struct mis_f {
    ZeroCounter* counters;
    uintE* perm;
    mis_f(ZeroCounter* _counters, uintE* _perm) : counters(_counters), perm(_perm) {}
...
inline sequence<bool> MaximalIndependentSet(Graph& G) {
    ...
    auto counters = parlay::tabulate<ZeroCounter>(n, [&](size_t i){
